...
Figure~\ref{fig:divergenceMagnitudeForce} illustrates the divergence and the magnitude scalar fields of the \emph{Force} vector dataset. Three force lines are drawn. The range of clamping for the divergence is -0.05 to 0.05 for the reason that divergence can have negative values. These two visualisations are quite common. This is not something that we did not expect, as the force affects only the neighbour data points from the points that it is applied. The rest data points remain zero. And that is true for both visualisations. However, there are also some differences. The centre points on the visualisation of magnitude (cf.~Figure~\ref{fig:divergenceMagnitudeForce}(b)) have the highest value and the rest scalar values around these points are decreasing. In the case of the visualisation of divergence (cf.~Figure~\ref{fig:divergenceMagnitudeForce}(a)), the force lines are composed of two parts. One high value part and one low value part. This is shown clear on the top force line in Figure~\ref{fig:divergenceMagnitudeForce}(a).
